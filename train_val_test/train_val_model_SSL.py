...
def train_classifier(data_loader, model, loss_function, optimizer, global_step, args, writer):
    ...
    for index, (inputs, labels) in enumerate(process):
        ...
        outputs, pretext_loss, mask_loss, jigsaw_T_loss, joint_loss, simloss = model(inputs)
        ...
        process.set_description(
            'Train: acc: {:4f}, loss: {:4f}, mask: {:4f}, PT: {:4f}, PS: {:4f}, sim: {:4f}, batch time: {:4f}, lr: {:4f}'.format(
                acc, ls, mask_loss, jigsaw_T_loss, joint_loss, simloss,
        ...
        if args.mode == 'train_val':
            ...
            writer.add_scalar('mask_ls', mask_loss, global_step)
            writer.add_scalar('jigT_ls', jigsaw_T_loss, global_step)
            writer.add_scalar('Joint_ls', joint_loss, global_step)
            writer.add_scalar('sim_ls', simloss, global_step)
...
def val_classifier(data_loader, model, loss_function, global_step, args, writer):
    ...
    for index, (inputs, labels, path) in enumerate(process):
        ...
        process.set_description(
            'Val-batch: acc: {:4f}, loss: {:4f}, mask: {:4f}, PT: {:4f}, PS: {:4f}, sim: {:4f}, time: {:4f}'.format(acc, ls, mask_loss, jigsaw_T_loss, joint_loss, simloss, process.iterable.last_duration))
