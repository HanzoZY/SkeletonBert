...
from model.bertnet import BertNet
from model.gptnet import GPTNet
from model.gptnet_Lay import GPTNet_Lay
from model.agcn_rebuild_TCN_continue import AGCN
...
def model_choose(args, block):
    ...
    if m == 'dstanet':
        ...
        model = BertNet(num_class=args.class_num, **args.model_param)
        shutil.copy2(inspect.getfile(BertNet), args.model_saved_name)
    elif m == 'agcn':
        model = AGCN(num_class=args.class_num, **args.model_param)
        shutil.copy2(inspect.getfile(AGCN), args.model_saved_name)
    elif m == 'gptnet':
        model = GPTNet(num_class=args.class_num, **args.model_param)
        shutil.copy2(inspect.getfile(GPTNet), args.model_saved_name)
    elif m == 'gptnet_lay':
        model = GPTNet_Lay(num_class=args.class_num, **args.model_param)
        shutil.copy2(inspect.getfile(GPTNet_Lay), args.model_saved_name)
    else:
...
def model_choose_DDP(args, block):
    m = args.model
    if m == 'dstanet':
        model = DSTANet(num_class=args.class_num, **args.model_param)
        shutil.copy2(inspect.getfile(DSTANet), args.model_saved_name)
