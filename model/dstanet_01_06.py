        ...
        if self.use_temporal_att:
            ...
            if self.att_t:
                q, k = torch.chunk(self.in_nett(z).view(N, 2 * self.num_subset, self.inter_channels, T, V), 2,
                                   dim=1)  # nctv -> n num_subset c'tv
                attention = attention + self.tan(
                    torch.einsum('nsctv,nscqv->nstq', [q, k]) / (self.inter_channels * V)) * self.alphat # attention shape: N,num_subset,V,V
            if self.glo_reg_t:
...
if __name__ == '__main__':
    ...
    # print(net(ske).shape)
    pe_s = PositionalEncoding(channel=16, joint_num=3, time_len=8, domain='spatial')
    pe_t = PositionalEncoding(channel=16, joint_num=3, time_len=8, domain='temporal')