...
class BertNet(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_frame=32, num_subset=3, dropout=0., num_person=2,
                 data_channel=3, layers_in_dim=[64], attentiondrop=0, dropout2d=0, out_channel=256, sample_for_pretext_rate=0.2, chose_rate4mask=0.1, mask_divide=[0.8, 0.2],num_seg=3, pretext_task_Train=False, context=False, global_S_A=True):
        ...
        # config=[[64, 64, 16, 1], [64, 64, 16, 1], [64, 128, 32, 1], [128, 256, 64, 1]]
        # config = [[64, 64, 16, 1], [64, 64, 16, 1], [64, 128, 32, 1],[128,128,32,1], [128, 256, 64, 1],[256,256,64,1]]
        config = [[64, 64, 16, 1], [64, 64, 16, 1], [64, 128, 32, 1], [128, 128, 32, 1], [128, 256, 64, 1],
                  [256, 256, 64, 1], [256, 256, 64, 1], [256, 256, 64, 1]]
        # self.linear_in = nn.Sequential(nn.Linear(in_features=in_channels,out_features=int(self.hiden_dim/2)),nn.ReLU(),nn.Linear(in_features=int(self.hiden_dim/2),out_features=self.hiden_dim), nn.ReLU())
        self.input_map = nn.Sequential(nn.Conv2d(3, 64, 1), nn.BatchNorm2d(64), nn.LeakyReLU(0.1))
        self.encoder = nn.ModuleList()
        num_frame = num_frame
        for index, (hiden_in, hiden_out, inter_channels, stride) in enumerate(config):
            self.encoder.append(
                STAttentionBlock(hiden_in, hiden_out, inter_channels, stride=stride, num_frame=num_frame,
                                 **param))
