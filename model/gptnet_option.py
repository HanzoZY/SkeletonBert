...
from model.pretext_task import MaskedPrediction, JigsawPrediction_T, ContrastiveLearning_SimSiam, Joint_Prediction, ReversePrediction, JigsawPrediction_T_labeled
import itertools
EPS = 1e-8
...
class GPTNet_option(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_frame=32, num_subset_S=3, num_subset_T=3, dropout=0., config=None, num_person=2,
        ...
        super(GPTNet_option, self).__init__()
        ...
        self.SSL_JigsawT = JigsawPrediction_T_labeled(hid_dim=self.out_channels, num_perm=len(self.permutations_T))
    ...
    def Jigsaw_T_generate_1(self,x):
        N, M, C, T, V = x.shape
        idx = list(range(T))
        cut_num = int(T/self.num_seg)
        cut_idx = np.array([idx[i*cut_num:(i+1)*cut_num] if i < self.num_seg-1 else idx[i*cut_num:] for i in range(self.num_seg)])
        ...
        x_T = torch.stack(x_list)
        assert num_perm == x_T.size(0)
        x_T = x_T.view(num_perm, N, M, C, T, V)
    ...
    def forward(self, x):
        ...
        if self.use_SSL == True:
            ...
            '''for the Jigsaw_T_generate function'''
            ...
            '''for the Jigsaw_T_generate_1 function'''
            # if self.SSL_option['pred_T'] == True:
            #     x_jigsaw_T = self.Jigsaw_T_generate_1(x=x_temp.view(N, M, self.in_channels, T, V))
            #     num_perm = x_jigsaw_T.size(0)
            #     assert num_perm == len(self.permutations_T)
            #     x_jigsaw_T = x_jigsaw_T.view(num_perm * N * M, self.in_channels, T, V)
            #     x_jigsaw_T = self.pet(self.pes(x_jigsaw_T))

            '''for the Jigsaw_T_generate_labeled function'''
            if self.SSL_option['pred_T'] == True:
            ...
            '''predict the Jigsaw T data_added'''
            # if self.SSL_option['pred_T'] == True:
            #     x_jigsaw_T = x_jigsaw_T.view(num_perm, N, M, self.out_channels, T*V)
            #     x_jigsaw_T = x_jigsaw_T.mean(-1).mean(2).view(num_perm, N, self.out_channels)
            #     jigsaw_T_loss = self.SSL_JigsawT(x=x_jigsaw_T)
            #     x_all.append(x_jigsaw_T)

            '''predict the Jigsaw T labeled'''
            if self.SSL_option['pred_T'] == True:
