...
class PositionalEmbedding_ours(nn.Module):

    def __init__(self, channel, joint_num, time_len, domain):
        super(PositionalEmbedding_ours, self).__init__()
        self.joint_num = joint_num
        self.time_len = time_len
    ...
    def forward(self, x):  # nctv
        ...
        x = x + pe
        return x
class STAttentionBlock_P(nn.Module):
    def __init__(self, in_channels, out_channels, inter_channels, num_subset=3, num_node=25, num_frame=32,
        ...
        super(STAttentionBlock_P, self).__init__()
        ...
        if use_spatial_att:
            ...
            self.pes = PositionalEmbedding_ours(in_channels, num_node, num_frame, 'spatial')
        ...
        if use_temporal_att:
            ...
            self.out_nett = nn.Sequential(
                nn.Conv2d(in_channels, out_channels, (7, 1), padding=(3, 0), bias=True, stride=(stride, 1)),
                nn.BatchNorm2d(out_channels),
            )
    ...
    def forward(self, x):
        ...
        if self.use_spatial_att:
            ...
            if self.use_pes:
                ...
                y_s = x
            if self.att_s:
                q, k = torch.chunk(self.in_nets(y_s).view(N, 2 * self.num_subset, self.inter_channels, T, V), 2,
            ...
            y_s = torch.einsum('nctu,nsuv->nsctv', [x, attention]).contiguous() \
                .view(N, self.num_subset * self.in_channels, T, V)
            y_s = self.out_nets(y_s)  # nctv

            y_s = self.relu(self.downs1(x) + y_s)

            y_s = self.ff_nets(y_s)

            y_s = self.relu(self.downs2(x) + y_s)
        else:
            y_s = self.out_nets(x)
            y_s = self.relu(self.downs2(x) + y_s)
        ...
        if self.use_temporal_att:
            ...
            if self.use_pet:
                ...
                y_t = x
            if self.att_t:
                q, k = torch.chunk(self.in_nett(y_t).view(N, 2 * self.num_subset, self.inter_channels, T, V), 2,
                                   dim=1)  # nctv -> n num_subset c'tv
                attention = attention + self.tan(
            ...
            attention = self.drop(attention)
            y_t = torch.einsum('nctv,nstq->nscqv', [x, attention]).contiguous() \
                .view(N, self.num_subset * self.in_channels, T, V)
            y_t = self.out_nett(y_t)  # nctv

            y_t = self.relu(self.downt1(x) + y_t)

            y_t = self.ff_nett(y_t)

            y_t = self.relu(self.downt2(x) + y_t)
        else:
            y_t = self.out_nett(x)
            y_t = self.relu(self.downt2(x) + y_t)
...
class BertNet(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_frame=32, num_subset=3, dropout=0., num_person=2,
        ...
        self.PE_T_B = PositionalEmbedding(d_model=self.hiden_dim)
        self.PE_S_B = nn.Parameter(torch.zeros(self.num_joint, self.hiden_dim))
        # nn.init.kaiming_normal_(self.PE_S, a=math.sqrt(5))
        nn.init.uniform_(self.PE_S)

        self.PE_T_D = PositionalEncoding(self.hiden_dim, self.num_joint, num_frame, 'temporal')
        self.PE_S_D = PositionalEmbedding_ours(self.hiden_dim, self.num_joint, num_frame, 'spatial')
    ...
    def forward(self, x):
        ...
        if self.training == True and self.pretext_task_Train == True:
            if self.use_dsta == True:
                ...
                x_predict_S = self.PE_T_D(x_predict_S.permute(0, 3, 1, 2).contiguous())
                x = self.PE_T_D(self.PE_S_D(x.permute(0, 3, 1, 2).contiguous()))
                x_masked = self.PE_T_D(self.PE_S_D(x_masked.permute(0, 3, 1, 2).contiguous()))
                x_jigsaw_T = self.PE_T_D(self.PE_S_D(x_jigsaw_T.permute(0, 3, 1, 2).contiguous()))
